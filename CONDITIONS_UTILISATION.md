# Conditions Générales d'Utilisation (CGU) - Portail Pasma-sys ENT

**En vigueur au 23 mai 2026**

Le présent document définit les conditions générales d'utilisation applicables aux utilisateurs (parents d’élèves, enseignants, personnels administratifs) accédant au **Portail Pasma-sys ENT** ainsi qu'au module de **Gestion de la Trésorerie APEE**.

## 1. Acceptation des Conditions
L’accès et l’utilisation de ce portail web impliquent l'acceptation sans réserve des présentes Conditions Générales d'Utilisation. Si un utilisateur refuse tout ou partie de ces règles, il est prié de cesser immédiatement l'usage de la plateforme.

## 2. Description des Services
Le Portail Pasma-sys est un outil de suivi scolaire et administratif qui propose :
* Un Espace Numérique de Travail (ENT) pour suivre la scolarité de l'élève (absences, notes, devoirs, rendez-vous enseignants).
* Un Espace APEE permettant l'enregistrement, la consultation des cotisations d'apurement, la vérification des reçus de caisse informatisés, ainsi que la réception d'alertes par canaux SMS, WhatsApp, et email.

## 3. Administration et Édition de la Plateforme
L'application est administrée et supervisée de manière unique par :

* **Éditeur / Administrateur :** Jacques Bene Mbama
* **Téléphone professionnel :** +237 656 454 053
* **E-mail professionnel :** jacquesbene301@gmail.com

## 4. Obligations des Utilisateurs
En accédant au portail, l’utilisateur s’engage à :
1. **Fournir des informations exactes :** Renseigner un nom réel, un numéro de téléphone valide et en service, ainsi qu'une adresse e-mail fonctionnelle pour les communications obligatoires.
2. **Confidentialité des accès :** Protéger l'accès à son compte utilisateur et signaler toute usurpation ou faille suspecte à l'administrateur.
3. **Usage loyal :** Ne pas exploiter de failles de sécurité, ne pas surcharger les formulaires d'importation de bases de données, et ne pas usurper l'identité d'un autre parent ou d'un gestionnaire d'école.

## 5. Responsabilité de l'Établissement & de l'APEE
* **Exactitude des chiffres :** Le bureau APEE sous la responsabilité de Jacques Bene Mbama s'efforce de garantir la fidélité des enregistrements en caisse. En cas d'écart entre un versement physique (reçu papier d'une banque affiliée) et l'affichage numérique, le reçu de caisse physique signé prévaut.
* **Fonctionnement technique :** L'administrateur décline toute responsabilité pour les dysfonctionnements passagers liés aux réseaux de télécommunication mobiles locaux ou aux serveurs de messagerie lors de la distribution automatisée des rappels de paiement.

## 6. Propriété Intellectuelle
L'ensemble de la charte graphique, de l'interface logicielle de trésorerie, du logo Pasma-sys ENT et des codes applicatifs constitutifs de ce portail sont la propriété intellectuelle exclusive de l'administrateur et de ses concepteurs techniques. Toute exploitation ou extraction non autorisée est interdite.

## 7. Modifications des Conditions
Les présentes conditions peuvent être modifiées ou enrichies à tout moment pour correspondre aux évolutions du service scolaire. Les utilisateurs seront informés de toute révision majeure par notification sur l’Espace ENT.

Pour toute réclamation, contestation ou pour obtenir une clarification, veuillez joindre l'adresse : **jacquesbene301@gmail.com** ou par téléphone au **+237 656 454 053**.
